	...
	async function loadModels() {
		...
		try {
			...
			models = modelData.models.map((m: any) => ({
				...m,
				size: sizeStr(
					m.artifacts?.reduce((a: number, art: any) => a + (art.size ?? 0), 0) ?? 0
				),
			}));
		} catch (e) {
	...
	{:else if models.length === 0}
		<Card.Root>
			<Card.Content class="flex flex-col items-center gap-2 py-10 text-center">
				...
				<p class="text-sm text-muted-foreground">No models installed yet.</p>
				<p class="text-xs text-muted-foreground">
					Go to the Search page to find and install models from HuggingFace.
				</p>
			</Card.Content>
		</Card.Root>
	{:else}
					...
					<Table.Body>
						{#each models as m (m.id)}
							<Table.Row>
								...
								<Table.Cell class="capitalize text-muted-foreground">{m.type}</Table.Cell>
								<Table.Cell class="tabular-nums text-muted-foreground">{m.size}</Table.Cell>
								<Table.Cell>
									...
									<div class="flex justify-end gap-1">
										...
										{#if activeModelId === m.id}
											<Button
												variant="outline"
												size="sm"
												onclick={() => postAction(`/api/v1/models/${m.id}/deactivate`)}
											>
										...
										<Button
											variant="ghost"
											size="sm"
											onclick={() => postAction(`/api/v1/models/${m.id}/remove`)}
										>
